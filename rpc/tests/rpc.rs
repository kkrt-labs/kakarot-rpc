...
mod tests {
    ...
    async fn test_get_block_by_hash_hydrated_is_ok() {
        ...
        let block = res.json::<EthJsonRpcResponse<Block>>().await;
        match block {
            Ok(block) => {
                // Header data
                ...
                assert_eq!(block.result.header.gas_used, U256::from(500000));
                assert_eq!(block.result.header.gas_limit, U256::from(1000000));
                assert_eq!(block.result.header.difficulty, U256::from(1000000));
                assert_eq!(block.result.header.size, Some(U256::from(100)));
                assert_eq!(block.result.header.base_fee_per_gas, U256::from(32));
                assert_eq!(block.result.header.mix_hash, H256::from_low_u64_be(0));
                assert_eq!(block.result.header.nonce, Some(H64::from_low_u64_be(0)));

                // Block
                assert_eq!(block.result.uncles, vec![]);
                // TODO: update tests when real data fetched
                assert_eq!(block.result.total_difficulty, U256::from(1000000));
                assert_eq!(block.result.size, Some(U256::from(100)));
                ...
                match transactions {
                    BlockTransactions::Full(transactions) => {
                        ...
                        if let Some(first_tx) = transactions.first() {
                            ...
                            let starknet_hash = FieldElement::from_hex_be(&"0x36b9fcadfafec68effe5c23bbacaf6197745a5e6317d3f174b80765942b5abb".to_string()).unwrap();
                            assert_eq!(
                                first_tx.hash,
                            ...
                            let starknet_signature_r = FieldElement::from_hex_be(&"0x5267c0d93467ddb5cfe0ab9db124ed5d57345e92a45111e7a08f8afa7666fae".to_string()).unwrap();
                            let starknet_signature_s = FieldElement::from_hex_be(&"0x622c1e743ae1060293085a9702ea1c6a7f642eb47b8eb9fb51ca0d156c5f5dd".to_string()).unwrap();
                            assert_eq!(
                                first_tx.r,
            ...
            Err(e) => println!("Error while deserializing block == {:?}", e),
        }
        server_handle.stop().unwrap();
    }
    ...
    // TODO: transaction_count on pending block ?
    // if calling starknet_getBlockTransactionCount then should return a value
    #[tokio::test]
    async fn test_block_transaction_count_by_hash_is_ok() {
        // TODO: call starknet_getBlockTransactionCount instead of starknet_getBlockWithTxHashes and then count in eth_getBlockTransactionCountByHash ?
        let (_, server_handle) = setup_rpc_server().await;
        let client = reqwest::Client::new();
        let res = client
            .post("http://127.0.0.1:3030")
