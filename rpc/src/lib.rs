...
mod eth_rpc;
use eth_rpc::{KakarotEthRpc, KakarotRpcServer};
use eyre::Result;
...
pub async fn run_server(starknet_client: StarknetClient) -> Result<SocketAddr> {
    ...
    let server = ServerBuilder::default().build("127.0.0.1:03030".parse::<SocketAddr>()?).await?;

    let addr = server.local_addr();

    let rpc_calls = KakarotEthRpc {starknet_client: starknet_client};
