...
async fn main() -> Result<()> {
    ...
    let kakarot_address = FieldElement::from_hex_be(&kakarot_address).map_err(|_| {
        eyre!("KAKAROT_ADDRESS should be provided as a hex string, got {kakarot_address}")
    })?;
    ...
    let proxy_account_class_hash = FieldElement::from_hex_be(&proxy_account_class_hash)
        .map_err(|_| {eyre!("PROXY_ACCOUNT_CLASS_HASH should be provided as a hex string, got {proxy_account_class_hash}")})?;

    let starknet_lightclient =
        KakarotClientImpl::new(&starknet_rpc, kakarot_address, proxy_account_class_hash)?;
