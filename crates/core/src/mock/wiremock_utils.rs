...
pub async fn setup_mock_client() -> Box<dyn KakarotEthApi<JsonRpcClient<HttpTransport>>> {
    ...
    let config = StarknetConfig::new(
        Some(Url::parse(&provider_url).unwrap()),
        kakarot_address,
...
fn mock_block_with_txs() -> Mock {
    ...
    let starknet_block_id: StarknetBlockId = EthBlockId::new(block_id).try_into().unwrap();
...
fn mock_block_with_txs_hashes() -> Mock {
    ...
    let starknet_block_id_tx_hashes: StarknetBlockId = EthBlockId::new(block_id_tx_hashes).try_into().unwrap();
